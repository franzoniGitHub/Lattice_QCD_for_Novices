...
#include "SETTINGS.h"
  ...
  exact->AddPoint(-0.5, 1.933);
  exact->AddPoint((double)N * 0.4 * a, 1.933);
  exact->SetName("exact");
  ...
  montecarlo->GetYaxis()->SetLimits(-2., 4.);
  montecarlo->GetYaxis()->SetRangeUser(-2., 4.);
  ...
  legend->AddEntry("exact", "Exact #DeltaE=1.933", "l");
